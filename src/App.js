...
const Box = ({position, args, color, speed}) => {
  ...
  return (
    ...
    <MeshWobbleMaterial attach="material" color={color} speed={speed} factor={0.8} />
    </animated.mesh>
  )
}
...
function App() {
  ...
  return (
    ...
    <Canvas shadowMap colorManagement camera={{position: [-5, 2, 10], fov: 60}}>
      ...
      <pointLight position={[0, -20, 0]} intensity={0.9} />
   ...
   <Box position={[0,1,0]} args={[3,2,1]} color={"lightblue"} speed={1} />
   <Box position={[-2,1,-5]} color="pink" speed={6} />
   <Box position={[5,1,-2]} color="pink" speed={7} />
   </group>
